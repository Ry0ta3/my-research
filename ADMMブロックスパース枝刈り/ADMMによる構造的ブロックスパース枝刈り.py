...
# ==============================================================================
# 色々な値を試せるようにリストで定義
prune_list = [80, 90, 95, 99]

# すべての組み合わせを生成
search_space = list(itertools.product(prune_list))
...
for i, (prune_rating, ) in enumerate(search_space):
    print("\n" + "="*80)
    print(f"グリッドサーチ試行: {i+1}/{len(search_space)}")
    print(f"パラメータ: prune_rating={prune_rating}")
    print("="*80)
