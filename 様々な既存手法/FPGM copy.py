...
# --- 2. スケジューラ関数を定義（これがWikiにあるルール！） ---
# 「今何ステップ目(i)で、全部で何ステップ(n)あるか」を受け取り、
# 「今の目標削減率」を返す関数を作って渡します。
def linear_scheduler(step, total_steps):
    # final_sparsity(0.87) までを線形に増やす計算
    final_sparsity = 0.4
    return final_sparsity * (step + 1) / total_steps

# --- 3. Pruner作成（ルール通りに引数をセット） ---
pruner = tp.pruner.MetaPruner(
    model,
    example_inputs,
    importance=imp,
    
    # 【重要ルール1】ここに「目標管理する関数」を渡す！
    # これで pruner.step() が勝手に計算してくれるようになります
    iterative_sparsity_scheduler=linear_scheduler,
    iterative_steps=iterative_steps,
    
    # 【重要ルール2】ResNetならこれは必須！
    # これがないと「制約違反」で0%になります
    global_pruning=True,
    
    # その他の必須設定
    pruning_ratio=0.87, # 最終目標
    ignored_layers=[],  # 全結合層も特別扱いしない
...
for i in range(iterative_steps):
    # 1. 少し削る (例: 全体の約8.7%分を進める)
    pruner.step()
    
    # 2. 【重要】再学習 (Fine-tuning)
    # 削ってバカになったモデルを、データセットで再教育して賢さを取り戻させる
