...
# ==============================================================================
# 色々な値を試せるようにリストで定義
rho_val_list = [0.01, 0.1, 1, 10]
gamma_val_list = [0.01, 0.1, 1, 10]


# すべての組み合わせを生成
search_space = list(itertools.product(rho_val_list, gamma_val_list))
...
for i, (rho_val, gamma_val) in enumerate(search_space):
    print("\n" + "="*80)
    print(f"グリッドサーチ試行: {i+1}/{len(search_space)}")
    print(f"パラメータ: rho_val={rho_val}, gamma_val={gamma_val}")
    print("="*80)
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            ...
            rho_val = rho_val        # block性ペナルティパラメータ
            gamma_val = gamma_val  # sparse性ペナルティパラメータ
